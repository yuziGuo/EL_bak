...
def set_args(predefined_dict_groups):
    ...
    args.high_level_clses = ['TAB', 'COL', 'CELL'] # ['TAB', 'COL', 'CELL', 'ROW', 'NL']
    args.option = {
        'tab' : ''
    ...
    args.embedding = 'bert'  # before: bert
...
class col_classifier(nn.Module):
    def __init__(self, args):
        ...
        self.encoder = TabEncoder(args)  # when initialized, encoder knows what table object it wants to get
    ...
    def forward(self, src, seg, tgt=None):
        output = self.encoder.encode(src, seg)['first-row']
        # output = self.encoder.encode(src, seg)['tab']
...
def evaluate(args, model, epoch_id=None):
    ...
    def get_result_for_one_test_set(args, ds_path, epoch_id):
        ds = microTableDataset(ds_path, train=False, shuffle_rows=args.shuffle_rows, moving_window_for_micro_table=args.moving_window_for_micro_table)
...
def train_and_eval(args, model):
    ds = microTableDataset(args.train_path, train=True, moving_window_for_micro_table=args.moving_window_for_micro_table)
    _loader = DataLoader(ds, batch_size=args.batch_size, collate_fn=fn_wrapper(args))
    args.train_steps = int(len(ds) * args.epochs_num/  (2* args.batch_size) ) + 1
    ...
    for epoch in range(1, args.epochs_num+1):
        ...
        # args.logger.info(optimizer.state_dict)
        for i, (src_batch, tgt_batch, seg_batch, _) in enumerate(_loader):
            # from demos.test_db_generate_seg_and_mask import check_segs
            # import ipdb; ipdb.set_trace()
            # check_segs(zip(seg_batch.tolist(), src_batch.tolist()), 'col')
            # import ipdb; ipdb.set_trace()
